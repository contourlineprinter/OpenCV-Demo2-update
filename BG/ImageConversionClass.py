...
class ImageConversion:
    ...
    def getImageReady(self, image):
        try:
            ...
            adaptThresImage = cv2.adaptiveThreshold(blurImage, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,
                                                    295, 1)
    ...
    def createContours(self, image, lineThickness=2):
        try:
            CANNY_THRESH_1 = 10
            CANNY_THRESH_2 = 200
            window_size = int(round(min(image.shape[0], image.shape[1]) * 0.05))  # Consider each w
            edges = cv2.Canny(image=image, threshold1=CANNY_THRESH_1, threshold2=CANNY_THRESH_2)
            edges = cv2.dilate(edges, None)
            edges = cv2.erode(edges, None)

            contours, hierarchy = cv2.findContours(edges, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # find countour
            ...
            self.filterPoints(contours, pointC, window_size )
            ...
            imageContourOld = cv2.drawContours(blankCanvas1, contours, -1, (0, 255, 0),
                                               lineThickness)  # draw the contour image with old point
            #imageContourNew = cv2.drawContours(blankCanvas2, [newContours], -1, (0, 255, 0),
            #                                   lineThickness)  # draw the contour image with new point

            imageContourNew = cv2.drawContours(blankCanvas2, pointC, -1, (0, 255, 0), lineThickness)

            # cv2.polylines(imageContourNew, newContours, isClosed=False, color=(0,255,0), thickness=lineThickness, lineType=8, shift=0)
    ...
    def filterPoints(self, contourPoints, newContourPoints, window_size):
        try:
            ...
            print("Initial Number of Objects: ", len(contourPoints))
            ...
            # Looping through it contour list
            for c in contourPoints:
                # We can use this epsilon instead of fixed 2 in approxPolyDP
                epsilon = 0.001 * cv2.arcLength(c, False)
                # approx = cv2.approxPolyDP(c, epsilon, True)
                # Apprying polygon approximation on the current contour.
                approx = cv2.approxPolyDP(c, 2, False)
                if not len(approx) == 4:
                    n_c.append(approx)

            # Printing current state of the contours.
            self.print_contours(n_c)

            for c in n_c:
                newContourPoints.append(c)

            print("Inital Number of Objects: ", len(newContourPoints))
            print("Last saved x: %d" % xsave)  # last save x
            print("Last saved y: %d" % ysave)  # last save y
            print(
            "\nNumber of points in old contour image: %d" % self.countPoints(contourPoints))  # number of points
            print("Number of points in contour image: %d\n" % self.countPoints(newContourPoints))  # number of points
    ...
    #   larger range - less points, less lines in the image
    # parameters: set of points that make up contour, range for x, range for y, minimum contour area accepted 
    def filterPointsOld(self, contourPoints, newContourPoints, rangeForX=7, rangeForY=7, minContourArea=200):
        try:
            contoursToDelete = []  # list of indexes of contours to delete
            ...
            p_x = []
            p_y = []
            # Looping through all original contour list
            for i in contourPoints:
                for j in i:
                    for k in j:
                        p_x.append(k[0])
            ...
            print("Inital Number of Objects: ", len(n_c))
            print("Last saved x: %d" % xsave)  # last save x
            print("Last saved y: %d" % ysave)  # last save y
            print("\nNumber of points in old contour image: %d" % self.countPoints(contourPoints))  # number of points
            print("Number of points in contour image: %d\n" % self.countPoints(n_c))  # number of points
